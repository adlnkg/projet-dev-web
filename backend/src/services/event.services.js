...
const EVENT_FIELD_VALIDATORS = {
    title: (value, definition) => normalizeTextValue(value, {
    ...
    organizer: (value, definition) => normalizeTextValue(value, {
        minLength: definition.minLength ?? 1,
        maxLength: definition.maxLength,
        fieldName: definition.label,
    }),
    ...
    price: (value, definition) => normalizeNumberValue(value, {
        min: definition.min,
        max: definition.max,
    ...
    maxParticipants: (value, definition) => normalizeNumberValue(value, {
        min: definition.min,
        max: definition.max,
        step: definition.step,
        integer: true,
...
const updateEvent = async (eventId, role, payload) => {
    ...
    if (generalUpdates.startTime && generalUpdates.endTime && generalUpdates.startTime >= generalUpdates.endTime) {
        const error = new Error("La date de debut doit etre strictement anterieure a la date de fin.");
        error.statusCode = 400;
        throw error;
    }

    if (generalUpdates.startTime && !generalUpdates.endTime && generalUpdates.startTime >= event.endTime) {
        const error = new Error("La date de debut doit etre strictement anterieure a la date de fin actuelle.");
        error.statusCode = 400;
        throw error;
    }

    if (!generalUpdates.startTime && generalUpdates.endTime && event.startTime >= generalUpdates.endTime) {
        const error = new Error("La date de fin doit etre strictement posterieure a la date de debut actuelle.");
        error.statusCode = 400;
        throw error;
    }

    if (generalUpdates.maxParticipants !== undefined && generalUpdates.maxParticipants < event.numberOfParticipants) {
        const error = new Error("La capacite maximale ne peut pas etre inferieure au nombre de participants actuel.");
        error.statusCode = 400;
        throw error;
    }
